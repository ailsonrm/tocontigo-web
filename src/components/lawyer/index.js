...
const PageLawyer = ({ lawyers, fetchLawyers, ownerId, fetchDashboardData }) => {
  const { currentUser, showSnackbar } = useContext(ContextUser);
  const { role } = currentUser;
  const [showModal, setShowModal] = useState(false);
  const [searchLawyerResult, setSearchLawyerResult] = useState([]);
  ...
  return (
    ...
    >
      <Search
        data={lawyers}
        fields={['name', 'email']}
        placeholder="Busque por advogados..."
        setSearchResult={setSearchLawyerResult}
      />
      {/* <ButtonGroup>
        <Button
